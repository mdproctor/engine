...
@ApplicationScoped
public class PlanItemCompletionEventBusAdapter {

  @Inject PlanItemCompletionHandler core;

  @ConsumeEvent(value = EventBusAddresses.WORKER_EXECUTION_FINISHED, blocking = true)
  void handleWorkerFinished(WorkflowExecutionCompleted event) {
    core.onWorkerFinished(event);
  }

  @ConsumeEvent(value = BlackboardEventBusAddresses.SUBCASE_EXECUTION_COMPLETED, blocking = true)
  void handleSubCaseFinished(SubCaseExecutionCompleted event) {
