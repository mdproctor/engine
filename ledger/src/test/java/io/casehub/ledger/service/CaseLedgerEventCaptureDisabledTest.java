...
/**
 * Verifies that {@link CaseLedgerEventCapture} does not write any ledger entry when {@code
 * casehub.ledger.enabled=false}.
 *
 * <p>The observer is called directly (same-package access) to avoid async delivery uncertainty.
 */
@QuarkusTest
@TestProfile(CaseLedgerEventCaptureDisabledTest.DisabledProfile.class)
class CaseLedgerEventCaptureDisabledTest {

  @Inject CaseLedgerEventCapture capture;
  @Inject CaseLedgerEntryRepository repository;

  @Test
  void disabledLedger_caseLifecycleEvent_writesNothing() {
