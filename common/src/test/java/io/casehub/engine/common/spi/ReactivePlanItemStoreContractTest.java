...
public abstract class ReactivePlanItemStoreContractTest {
  ...
  void save_and_findByCaseId() {
    ...
    run(store().save(request(caseId, planItemId, TaskStatus.PENDING), TEST_TENANT));
  ...
  void updateStatus_changes_stored_status() {
    ...
    String planItemId = UUID.randomUUID().toString();
    run(store().save(request(caseId, planItemId, TaskStatus.PENDING), TEST_TENANT));
    run(store().updateStatus(planItemId, TaskStatus.RUNNING));
    List<PlanItemRecord> results = run(store().findByCaseId(caseId, TEST_TENANT));
  ...
  void findAllDelegated_returns_delegated_across_cases() {
    ...
    run(store().save(request(case1, id1, TaskStatus.DELEGATED), TEST_TENANT));
    run(store().save(request(case2, id2, TaskStatus.DELEGATED), TEST_TENANT));
