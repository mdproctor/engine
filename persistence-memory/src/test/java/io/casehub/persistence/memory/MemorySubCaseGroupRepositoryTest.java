...
class MemorySubCaseGroupRepositoryTest {
  ...
  void getOrCreate_createsGroup() {
    SubCaseGroup g =
        repo.getOrCreate(parentId, groupId, 3, 2, OnThresholdReached.KEEP, "test-tenant")
            .await()
            .indefinitely();
  ...
  void registerChild_and_findByChildCaseId() {
    ...
    repo.getOrCreate(parentId, groupId, 3, 2, OnThresholdReached.KEEP, "test-tenant")
        .await()
        .indefinitely();
    repo.registerChild(parentId, groupId, childId, "test-tenant").await().indefinitely();
    Optional<SubCaseGroup> found =
        repo.findByChildCaseId(childId, "test-tenant").await().indefinitely();
  ...
  void findByChildCaseId_unknownChild_returnsEmpty() {
    Optional<SubCaseGroup> found =
        repo.findByChildCaseId(UUID.randomUUID(), "test-tenant").await().indefinitely();
  ...
  void incrementCompleted_updatesCount() {
    repo.getOrCreate(parentId, groupId, 3, 2, OnThresholdReached.KEEP, "test-tenant")
        ...
        .indefinitely();
    SubCaseGroup g =
        repo.incrementCompleted(parentId, groupId, "test-tenant").await().indefinitely();
  ...
  void incrementRejected_updatesCount() {
    repo.getOrCreate(parentId, groupId, 3, 2, OnThresholdReached.KEEP, "test-tenant")
        ...
        .indefinitely();
    SubCaseGroup g =
        repo.incrementRejected(parentId, groupId, "test-tenant").await().indefinitely();
  ...
  void markPolicyTriggered_setsFlag() {
    repo.getOrCreate(parentId, groupId, 3, 2, OnThresholdReached.KEEP, "test-tenant")
        .await()
        .indefinitely();
    repo.markPolicyTriggered(parentId, groupId, "test-tenant").await().indefinitely();
    SubCaseGroup g =
        repo.getOrCreate(parentId, groupId, 3, 2, OnThresholdReached.KEEP, "test-tenant")
            .await()
            .indefinitely();
