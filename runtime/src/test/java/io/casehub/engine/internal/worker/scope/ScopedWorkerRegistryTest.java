...
class ScopedWorkerRegistryTest {
  ...
  void register_and_get() {
    ...
    var session = reinvokedSession(caseId, "binding-a", "pi-1", LifecycleScope.COMPOUND, Participation.PARTICIPANT);
  ...
  void terminateByCase_removes_all_sessions_for_case() {
    UUID caseId = UUID.randomUUID();
    var s1 = reinvokedSession(caseId, "b1", "pi-1", LifecycleScope.COMPOUND, Participation.PARTICIPANT);
  ...
  void terminateByCase_does_not_affect_other_cases() {
    ...
    var sessionA = reinvokedSession(caseA, "b1", "pi-1", LifecycleScope.COMPOUND, Participation.PARTICIPANT);
    var sessionB = reinvokedSession(caseB, "b1", "pi-2", LifecycleScope.COMPOUND, Participation.PARTICIPANT);
  ...
  void register_replaces_existing_session() {
    UUID caseId = UUID.randomUUID();
    var s1 = reinvokedSession(caseId, "b1", "pi-1", LifecycleScope.COMPOUND, Participation.PARTICIPANT);
    var s2 = reinvokedSession(caseId, "b1", "pi-2", LifecycleScope.COMPOUND, Participation.PARTICIPANT);
  ...
  void terminateByScope_removes_only_owned_bindings() {
    UUID caseId = UUID.randomUUID();
    var s1 = reinvokedSession(caseId, "owned", "pi-1", LifecycleScope.COMPOUND, Participation.PARTICIPANT);
    var s2 = reinvokedSession(caseId, "unowned", "pi-2", LifecycleScope.CASE, Participation.COMPANION);
