...
class PersonalitySignalRecorderTest {
  ...
  void reinforcement_dominantMatchesDemand_recordsDominant() {
    var profile =
        List.of(new DispositionValue("Ti", 0.5), new DispositionValue("Ne", 0.3));
  ...
  void checkReflection_aligned_noReflection() {
    ...
    when(dispositionHealth.probe(any(), any()))
            .thenReturn(new io.casehub.eidos.api.DispositionHealth.DispositionStatus.Aligned(java.util.Map.of()));
