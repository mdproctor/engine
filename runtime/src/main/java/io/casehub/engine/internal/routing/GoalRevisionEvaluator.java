...
public class GoalRevisionEvaluator {
  ...
  public void record(
    ...
    if (!enabled) return;
    if (!goalSignalStore.isResolvable()) return;
    if (!goalEvolution.isResolvable()) return;
    ...
    states.compute(
        key,
        (k, state) -> {
          if (state == null) state = new RevisionState();
          state.outcomeCount++;
  ...
  private List<AgentGoal> mergeDescriptions(List<AgentGoal> goals, GoalRevisionProposal proposal) {
    ...
    if (descriptionsByGoal.isEmpty()) return goals;
